...
# For each judge model, the single closest proxy by benchmark scores
# (MATH500 / MMLU / MBPP+), excluding the judge itself.
# Scores sourced from official model cards, Qwen blog, and third-party papers:
#   gemma-2-9b:    MATH500=70.1, MMLU=65.2, MBPP+=55.4
#   gemma-2-27b:   MATH500=70.2, MMLU=65.5, MBPP+=70.2
#   qwen-2.5-3b:   MATH500=78.2, MMLU=50.7, MBPP+=53.2
#   qwen-2.5-7b:   MATH500=76.0, MMLU=74.2, MBPP+=62.9
#   qwen-2.5-14b:  MATH500=77.0, MMLU=79.7, MBPP+=63.2
#   qwen-2.5-32b:  MATH500=82.0, MMLU=83.7, MBPP+=67.2
#   qwen-2.5-72b:  MATH500=94.3, MMLU=78.9, MBPP+=82.5
#   llama-3.2-3b:  MATH500=54.6, MMLU=41.0, MBPP+=45.6
#   llama-3.1-8b:  MATH500=51.9, MMLU=69.4, MBPP+=72.8
#   llama-3.1-70b: MATH500=81.8, MMLU=85.3, MBPP+=86.9
#   llama-3.3-70b: MATH500=79.6, MMLU=86.0, MBPP+=87.6
CLOSEST_PROXY_PER_JUDGE = {
    "gemma-2-9b":    "gemma-2-27b",    
    "gemma-2-27b":   "gemma-2-9b",     
    "qwen-2.5-3b":   "gemma-2-9b",     
    "qwen-2.5-7b":   "qwen-2.5-14b",   
    "qwen-2.5-14b":  "qwen-2.5-7b",    
    "qwen-2.5-32b":  "qwen-2.5-14b",   
    "qwen-2.5-72b":  "llama-3.1-70b", 
    "llama-3.2-3b":  "qwen-2.5-3b",    
    "llama-3.1-8b":  "gemma-2-27b",    
    "llama-3.1-70b": "llama-3.3-70b",  
    "llama-3.3-70b": "llama-3.1-70b",  
}


TOP2_PROXIES_PER_JUDGE = {
    "gemma-2-9b":    {"gemma-2-27b", "qwen-2.5-7b"},
    "gemma-2-27b":   {"gemma-2-9b",  "qwen-2.5-7b"},
    "qwen-2.5-3b":   {"gemma-2-9b",  "qwen-2.5-7b"},
    "qwen-2.5-7b":   {"qwen-2.5-14b","qwen-2.5-32b"},
    "qwen-2.5-14b":  {"qwen-2.5-7b", "qwen-2.5-32b"},
    "qwen-2.5-32b":  {"qwen-2.5-14b","qwen-2.5-7b"},
    "qwen-2.5-72b":  {"llama-3.1-70b","llama-3.3-70b"},
    "llama-3.2-3b":  {"qwen-2.5-3b", "gemma-2-9b"},
    "llama-3.1-8b":  {"gemma-2-27b", "qwen-2.5-7b"},
    "llama-3.1-70b": {"llama-3.3-70b","qwen-2.5-32b"},
    "llama-3.3-70b": {"llama-3.1-70b","qwen-2.5-32b"},
}
...
def setup_logging(output_dir: Path) -> logging.Logger:
    ...
    logger.info("=" * 80)
    logger.info("JUDGE SWAP ANALYSIS - MEAN-OF-DIFFERENCES VERSION (CLOSEST PROXY ONLY)")
    logger.info("=" * 80)
...
def load_cached_preferences(cache_dir: Path, logger: logging.Logger) -> Dict:
    ...
    for judge_dir in cache_dir.iterdir():
        ...
        for ref_dir in judge_dir.iterdir():
            ...
            # Create entries for each proxy (only keep the top-2 closest proxies per judge)
            for proxy_name, k_data in proxies_k_data.items():
                allowed = TOP2_PROXIES_PER_JUDGE.get(judge)
                if allowed is not None and proxy_name not in allowed:
                    logger.debug(f"Skipping proxy={proxy_name} for judge={judge} (top-2 proxies are {allowed})")
                    continue
...
def load_author_obfuscation_cache(quality_dir: Path, logger: logging.Logger) -> Dict:
    ...
    for judge_dir in quality_dir.iterdir():
        ...
        for json_file in judge_dir.glob("*.json"):
            ...
            allowed = TOP2_PROXIES_PER_JUDGE.get(judge)
            if allowed is not None and proxy not in allowed:
                logger.debug(f"Skipping proxy={proxy} for judge={judge} (top-2 proxies are {allowed})")
                continue
...
def main():
    ...
    # Output to jsd_closest_proxy_{results_dir} to keep results separate
    base_results_dir = Path(args.results_dir).name
    if args.output_dir:
        output_dir = Path(args.output_dir)
    else:
        output_dir = Path(f"jsd_top2_proxy_{base_results_dir}") / dataset / "analysis"
