...
def prepare_scatter_data(judge_ref_accuracy, self_pref_data=None):
    ...
    for judge in judges:
        ...
        result[judge] = {
            'task_accuracy': task_accuracy * 100,
            'family': family,
            'size': size,
        }
        
        # Add self-preference data with flexible matching
        sp_data = None
        if self_pref_data:
            ...
            elif judge_lower in {k.lower() for k in self_pref_data.keys() if not k.startswith('_')}:
                for sp_key in self_pref_data.keys():
                    if sp_key.startswith('_'):
                        continue
                    if sp_key.lower() == judge_lower:
                        sp_data = self_pref_data[sp_key]
                        break
            # 3. Try partial match (e.g., 'GPT-3.5' matches 'gpt-3.5-turbo')
            else:
                for sp_key in self_pref_data.keys():
                    if sp_key.startswith('_'):
                        continue
                    sp_key_lower = sp_key.lower()
                    if judge_lower in sp_key_lower or sp_key_lower in judge_lower:
                        sp_data = self_pref_data[sp_key]
                        logger.info(f"Partial match: '{judge}' -> '{sp_key}'")
                        break
        ...
        if sp_data:
            ...
            logger.warning(f"No self-pref match found for judge: {judge}")
